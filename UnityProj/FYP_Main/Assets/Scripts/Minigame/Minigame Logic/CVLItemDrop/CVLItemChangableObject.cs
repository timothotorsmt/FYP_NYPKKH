...
public class CVLItemChangableObject : MonoBehaviour
{
    // List out all needed sprites
    [SerializeField] private List<changableObject<CVLItemTasks>> _changableObjects;    

    // Start()
    public void Start() 
    {
        // Subscribe to the current task state
        CVLItemTaskController.Instance.CurrentTask.Value.Subscribe(State => {
