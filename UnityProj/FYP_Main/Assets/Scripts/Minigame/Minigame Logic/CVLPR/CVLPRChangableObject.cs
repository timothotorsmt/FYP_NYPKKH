...
public class CVLPRChangableObject : MonoBehaviour
{
    // List out all needed sprites
    [SerializeField] private List<changableObject<CVLPRTasks>> _changableObjects;    

    // Start()
    public void Start() 
    {
        // Subscribe to the current task state
        CVLPRTaskController.Instance.CurrentTask.Value.Subscribe(State => {
